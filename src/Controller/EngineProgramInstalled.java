...
public class EngineProgramInstalled {
	...
	public void List_program_terinstall(String topic_setting,int qos_message_command) {
		try {
			...
			System.err.println("SUBS : "+rtopic_validasi_command);
			client_transreport.subscribe(rtopic_validasi_command, qos_message_command, new IMqttMessageListener() {
				...
				public void messageArrived(final String topic, final MqttMessage message) throws Exception {
						...
						if(topic.contains("BYLINE")) {
							...
							String command_cmd = Parser_COMMAND;// "powershell.exe "+Parser_COMMAND.replace("\\\"get-childitem", "\"get-childitem").replace("}\\\"", "}\"");
							System.out.println("command_cmd : "+command_cmd);
							/*
							String list_ip = "";
							int jumlah_station = 0;
							
							if(Parser_HASIL == "IS_INDUK") {
								
								String query_is_induk = "SELECT IP FROM tokomain where KDCAB = '"+Parser_CABANG+"' AND IS_INDUK = '1';";
								String list_ip_is_induk = gf.GetTransReport(query_is_induk,1, false);
								String sp_record[] = list_ip_is_induk.split("~");
								jumlah_station = sp_record.length;
								for(int i = 0;i<sp_record.length;i++) {
									String sp_field[] = sp_record[i].split("%");
									String ip = sp_field[0];
								...
								String query_is_induk = "SELECT IP FROM tokomain where KDCAB = '"+Parser_CABANG+"' AND IS_INDUK = '0';";
								String list_ip_is_induk = gf.GetTransReport(query_is_induk,1, false);
								String sp_record[] = list_ip_is_induk.split("~");
                			...
                			obj.put("COMMAND_CMD",command_cmd);
                			obj.put("LIST_IP",list_ip);
                			Parser_COMMAND = obj.toJSONString();
                			*/
		                    ...
		                    //System.out.println("res_message_otp : "+res_message);
